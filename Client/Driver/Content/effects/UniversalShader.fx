...
float CalculateVertexLightingFactor(float3 inputNormal)
{
    ...
    if(xEnableLighting)
    {
	   ...
	   float3 lightDirection = normalize(-xLightView._13_23_33);
...
ShadowMapOutput VertexShadowMap(float4 position : POSITION)
{
    ...
    float4 viewPosition = mul(WorldPosition(position), xLightView);
	output.Position = mul(viewPosition, xLightProjection);
...
float3 CalculateFog(float3 foglessColor, float4 worldPosition)
{
    ...
    float3 fogColor = saturate(float3(0.85, 0.85, 1.0) * xAmbient);
