...
SET SLICE_COMPILER=%ICEROOT%\bin\slice2cpp

:: Directory where the *.ice files reside.
SET SLICE_DIR=.

:: Loop through the directory which has the slice code in it and compile every Ice file.
FOR /F %%a IN ('dir /b %SLICE_DIR%\*.ice') DO %SLICE_COMPILER% --stream -I%SLICE_DIR% -I%ICEROOT%\slice --output-dir=%SLICE_DIR% %SLICE_DIR%\%%a && IF [%QUIET%]==[0] echo Success: %%a || echo Failure: %%a
